...
function getDeliveryCostForPaymentIntent(deliveryMethod, legacyDeliveryCost) {
  ...
  if (deliveryCost === DELIVERY_COST_BY_METHOD["Made to Order"] ||
      deliveryCost === DELIVERY_COST_BY_METHOD["Regular Delivery"] ||
...
function computeExpectedAmount(subtotal, deliveryCost, discountAmount = 0, currency = "kwd") {
  if (String(currency).toLowerCase() !== "kwd") {
    throw new Error("Unsupported currency.");
  }

  const total = Number(subtotal) + Number(deliveryCost) - Number(discountAmount || 0);
