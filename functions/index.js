...
async function getVerifiedPaymentIntent(stripe, paymentIntentId) {
  try {
...
async function refundUnusedPaymentIntent(stripe, paymentIntentId, uid, reason) {
  try {
    const lockSnap = await db.collection("payment_intents").doc(paymentIntentId).get();
    if (lockSnap.exists) return;

    const paymentIntent = await getVerifiedPaymentIntent(stripe, paymentIntentId);
    const mismatch = paymentIntentMismatchReason({
      paymentIntent,
      uid,
      expectedAmount: Number(paymentIntent.amount),
    });
    if (mismatch) return;
    ...
    logger.warn("Refunded unused payment after order failure", {
      paymentIntentId,
      uid,
      reason: reason && reason.message || String(reason),
    });
  } catch (refundError) {
    logger.error("Failed to refund unused payment intent", {
      paymentIntentId,
      uid,
      refundError,
      orderError: reason,
...
exports.createPaymentIntent = onCall(async (request) => {
  ...
  try {
    ...
    const quote = await db.runTransaction((tx) => buildOrderQuote({
      items,
      deliveryMethod,
      discountCodeId,
    ...
    const amount = kwdToStripeAmount(quote.total);
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      metadata: {
        uid: request.auth.uid,
        orderTotalKwd: String(quote.total),
...
exports.createOrder = onCall(async (request) => {
  ...
  const data            = request.data || {};
  const items           = data.items;
  const deliveryMethod  = normalizeDeliveryMethod(data.deliveryMethod, data.deliveryCost);
  ...
  let orderResult;
  try {
    orderResult = await db.runTransaction(async (tx) => {
      const paymentIntentSnap = await tx.get(paymentIntentRef);
      if (paymentIntentSnap.exists) {
        const lockData = paymentIntentSnap.data() || {};
        if (lockData.customerUid === uid && lockData.orderNumber) {
          return { orderNumber: lockData.orderNumber, alreadyConsumed: true };
        }
      ...
      const expectedAmount = kwdToStripeAmount(quote.total);
      const paymentMismatch = paymentIntentMismatchReason({
        paymentIntent,
        uid,
        expectedAmount,
      });
      if (paymentMismatch) {
        throw new HttpsError("failed-precondition", paymentMismatch);
      }
      ...
      return { orderNumber, alreadyConsumed: false };
    });
  } catch (error) {
    await refundUnusedPaymentIntent(stripe, paymentIntentId, uid, error);
    throw error;
  }
